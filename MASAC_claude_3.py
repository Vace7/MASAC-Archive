...
class MASACAgent:
    def __init__(self, n_agents, obs_dims, act_dims, action_bound=1, gamma=0.99, polyak=0.995, lr=3e-4, alpha=0.2, autotune=True, critic_hidden_sizes=(256,256), actor_hidden_sizes=(64,64), use_layer_norm=False, utd_ratio=1, verbose=True, name="MASAC"):
        ...
        # Initialize separate alpha parameters for each agent
        if autotune:
            self.target_entropies = [-tf.reduce_prod(1).numpy() for _ in range(n_agents)]
            ...
            self.alphas = [tf.Variable(alpha, dtype=tf.float32, trainable=False) for _ in range(n_agents)]

        # Initialize actors and critics for each agent
        self.actors = [Actor(action_dim=act_dims, hidden_sizes=actor_hidden_sizes, action_scale=action_bound) 
    ...
    def train(self, batch):
        """Optimized training function using multi-head critic network with per-agent alpha"""
        ...
        curr_policies = tf.stack(curr_pis, axis=1)
        next_policies = tf.stack(next_pis, axis=1)
        curr_logprobs = tf.stack(curr_logps, axis=1)
        ...
        for _ in range(self.utd_ratio):
            with tf.GradientTape(persistent=True) as tape:
                ...
                # Compute targets for each agent using their respective alphas
                next_q_min = tf.minimum(next_q1, next_q2)
                # Stack alphas for broadcasting
                stacked_alphas = tf.stack(self.alphas)
                q_targets = tf.stop_gradient(
                    rews[:, tf.newaxis] + 
                    self.gamma * (1 - dones[:, tf.newaxis]) * 
                    (next_q_min - tf.reshape(stacked_alphas, [1, -1]) * next_logprobs)
                )
        ...
        # Update alphas for each agent
        if self.autotune:
            for agent_id in range(self.n_agents):
                with tf.GradientTape() as alpha_tape:
                    agent_entropy = tf.reduce_mean(curr_logprobs[:, agent_id])
                    alpha_loss = -tf.exp(self.log_alphas[agent_id]) * (
                        agent_entropy + self.target_entropies[agent_id]
                    )
                    
                alpha_grad = alpha_tape.gradient(alpha_loss, [self.log_alphas[agent_id]])
                self.alpha_optimizers[agent_id].apply_gradients(
                    zip(alpha_grad, [self.log_alphas[agent_id]])
                )
        ...
        for agent_id in range(self.n_agents):
            with tf.GradientTape() as pi_tape:
                ...
                updated_policy_actions = tf.concat([
                    policy_actions[:, :agent_id],
                    curr_pi[:, tf.newaxis],
                    policy_actions[:, agent_id+1:]
                ], axis=1)
                ...
                min_q_pi = tf.minimum(q1_pi, q2_pi)[:, agent_id]
                
                # Use agent-specific alpha for policy loss
                pi_loss = tf.reduce_mean(self.alphas[agent_id] * logp_pi - min_q_pi)
        ...
        return tf.reduce_mean(pi_losses), critic_loss, tf.reduce_mean(self.alphas)